...
class LngDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if item.name.startswith("HI_"):
            y = 0
        elif item.name.startswith("BA_"):
            y = 1
        elif item.name.startswith("TA_"):
            y = 2
        else:
...
def infer(model, inputs, *, labels=( "HI","BA","TA")):
    ...
    for audio in inputs:
        with soundfile.SoundFile(audio) as sf:
            out = []
            for signal in catch(sf.blocks(sf.samplerate * 3, fill_value=0)):
                # Downmix to mono
                if signal.ndim > 1:
